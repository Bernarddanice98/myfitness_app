  ...
  try {
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 1000,
        temperature: 0.7,
        messages: [{ role: 'system', content: `Create a healthy recipe for ${goal}. Return ONLY JSON with: name, calories, protein, carbs, fat, ingredients (array), instructions (array).` }, { role: 'user', content: `Create a recipe for ${goal}.` }]
      })
    });
    const data = await response.json();
    let aiResponse = data.choices[0]?.message?.content || '';
...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    await pool.query('INSERT INTO user_data (user_id, profile, saved_workouts, workout_history, completed_workouts) VALUES ($1, $2, $3, $4, $5)', [user.id, {}, [], {}, {}]);
...
app.post('/api/demo-login', async (req, res) => {
  try {
    ...
    if (demoUser.rows.length === 0) {
      ...
      demoUser = newUser;
      // ✅ CORRECTION ICI
      await pool.query('INSERT INTO user_data (user_id, profile, saved_workouts, workout_history, completed_workouts) VALUES ($1, $2, $3, $4, $5)', [demoUser.rows[0].id, {}, [], {}, {}]);
    }
  ...
  } catch (error) { console.error('Demo login error:', error); res.status(500).json({ error: 'Demo login failed' }); }
});
...
app.get('/api/verify', (req, res) => { res.json({ valid: true, user: req.user }); });
...
app.get('/api/user-data/:userId', async (req, res) => {
  ...
  try {
    ...
    res.json({ profile: result.rows[0].profile || {}, savedWorkouts: result.rows[0].saved_workouts || [], workoutHistory: result.rows[0].workout_history || {} });
...
app.post('/api/user-data/:userId', async (req, res) => {
  ...
  if (req.user.userId.toString() !== userId) return res.status(403).json({ error: 'Access denied' });
  try {
    await pool.query(`INSERT INTO user_data (user_id, profile, saved_workouts, workout_history, updated_at) VALUES ($1, $2, $3, $4, CURRENT_TIMESTAMP) ON CONFLICT (user_id) DO UPDATE SET profile = EXCLUDED.profile, saved_workouts = EXCLUDED.saved_workouts, workout_history = EXCLUDED.workout_history, updated_at = CURRENT_TIMESTAMP`, [userId, profile || {}, savedWorkouts || [], workoutHistory || {}]);
...
app.post('/api/workout-progress/:userId', async (req, res) => {
  ...
  try {
    ...
    workoutHistory[today].push({ id: Date.now(), workoutName: workoutData?.day?.name || 'Workout', timestamp: new Date().toISOString(), sets: setsStatus || {}, weights: weightsData || {}, reps: repsData || {} });
    await pool.query(`INSERT INTO user_data (user_id, workout_history, updated_at) VALUES ($1, $2, CURRENT_TIMESTAMP) ON CONFLICT (user_id) DO UPDATE SET workout_history = EXCLUDED.workout_history, updated_at = CURRENT_TIMESTAMP`, [userId, workoutHistory]);
    res.json({ success: true });
  } catch (error) { res.status(500).json({ error: error.message }); }
});
...
app.get('/api/completed-workouts/:userId', async (req, res) => {
  ...
  try {
    ...
    if (userDataResult.rows.length === 0) {
      // ✅ CORRECTION ICI
      await pool.query('INSERT INTO user_data (user_id, profile, saved_workouts, workout_history, completed_workouts) VALUES ($1, $2, $3, $4, $5)', [userId, {}, [], {}, {}]);
    }
    ...
    if (typeof completedWorkouts === 'string') try { completedWorkouts = JSON.parse(completedWorkouts); } catch(e) { completedWorkouts = {}; }
...
app.post('/api/workout-complete/:userId', async (req, res) => {
  ...
  try {
    ...
    if (!alreadyCompleted) completedWorkouts[weekKey].push({ workoutName, dayIndex, completedAt: new Date().toISOString() });
...
async function resetWeeklyWorkouts() {
  ...
  } catch (error) { console.error('Weekly reset error:', error); }
}
